...
public class Poker {
	...
	private int getScore(Card[] hand){
		...
		for(Card card : hand){
			...
			if(card.suit != flushSuit){
				flushSuit = -1; // Different suits hence no flush
			}
		}
		
		int[] rankCountList = new int[6];
		for(i=0;i<rankCounts.length;i++){
			rankCountList[rankCounts[i]]++;
		}
		int bestCards = get5FrequentHighestCards(rankCounts, rankCountList);
		int straightHighRank = getStraightHighestRank(rankCounts);
		
		if (straightHighRank != -1 && flushSuit != -1 ) return 8 << 20 | straightHighRank; // Straight flush
		else if (rankCountList[4] == 1 ) return 7 << 20 | bestCards; // Four of a kind
		else if (rankCountList[3] == 1 && rankCountList[2] == 1) return 6 << 20 | bestCards; // Full house
		else if (flushSuit != -1 ) return 5 << 20 | bestCards; // Flush
		else if (straightHighRank != -1 ) return 4 << 20 | straightHighRank; // Straight
		else if (rankCountList[3] == 1 ) return 3 << 20 | bestCards; // Three of a kind
		else if (rankCountList[2] == 2 ) return 2 << 20 | bestCards; // Two pairs
		else if (rankCountList[2] == 1 ) return 1 << 20 | bestCards; // One pair
		else return 0 << 20 | bestCards; // High card
		
	}
	
	private int get5FrequentHighestCards(int[] ranks, int[] rankList){
		int result = 0;
		int count = 0;
	...
	private int getStraightHighestRank(int[] ranks){
		outer:
